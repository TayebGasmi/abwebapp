...
import com.appointment.booking.model.GlobalFilter;
...
public class FilterUtil {
    ...
    public static <T> Specification<T> filter(Map<String, List<FilterModel>> filters) {
        return (root, query, criteriaBuilder) -> criteriaBuilder.and(filters.entrySet().stream().map(
                entry -> createPredicateForEntry(entry.getKey(), entry.getValue(), criteriaBuilder, root))
            .toArray(Predicate[]::new));
    }

    public static <T> Specification<T> filter(Map<String, List<FilterModel>> filters,
        GlobalFilter globalFilter) {
        return (root, query, criteriaBuilder) -> filter(filters).and(
                createGlobalFilterSpecification(globalFilter))
            .toPredicate((Root<Object>) root, query, criteriaBuilder);
    }

    public static <T> Specification<T> createGlobalFilterSpecification(GlobalFilter globalFilter) {

        List<FilterModel> filterModels = List.of(
            new FilterModel(MatchMode.CONTAINS, globalFilter.getValue(), Operator.OR));

        return (root, query, criteriaBuilder) -> criteriaBuilder.or(globalFilter.getKeys().stream()
            .map(key -> createPredicateForEntry(key, filterModels, criteriaBuilder, root))
            .toArray(Predicate[]::new));
    }

    private static Predicate createPredicateForEntry(String key, List<FilterModel> filterModels,
        CriteriaBuilder criteriaBuilder, Root root) {
        Predicate[] predicates = filterModels.stream()
            .map(filterModel -> createPredicateFromProperty(key, filterModel, criteriaBuilder, root))
            .toArray(Predicate[]::new);

        return filterModels.get(0).getOperator() == Operator.AND ? criteriaBuilder.and(predicates)
            : criteriaBuilder.or(predicates);
    }

    public static Predicate createPredicateFromProperty(String property, FilterModel filterModel,
        CriteriaBuilder criteriaBuilder, Root root) {

        Path propertyTextPath = definePropertyPath(root, property);
        return applyFilter(propertyTextPath, filterModel.getValue(), filterModel.getMatchMode(),
            criteriaBuilder);
    }
    ...
    private static <T> Path<Object> getPropertyPath(Root<T> root, String[] properties,
        Path<Object> propertyPath) {
    ...
    private static <T> Predicate applyFilter(Path<T> propertyPath, String filterValue,
        MatchMode textFilterMatchMode, CriteriaBuilder criteriaBuilder) {
        if (filterValue == null || filterValue.isBlank()) {
            return criteriaBuilder.isTrue(criteriaBuilder.literal(true));
        }
        if (isDate(propertyPath.getJavaType())) {

            return handleDateFilter(propertyPath, filterValue, textFilterMatchMode, criteriaBuilder);
        }
        if (isNumericPath(propertyPath.getJavaType())) {
    ...
    private static <T> Expression<String> buildPropertyExpression(Path propertyPath,
        CriteriaBuilder criteriaBuilder) {
        return criteriaBuilder.lower(propertyPath);
    }

    private static <T> Predicate handleNumericFilter(Path property, String filterValue,
        MatchMode textFilterMatchMode, CriteriaBuilder criteriaBuilder) {
        switch (textFilterMatchMode) {
            case CONTAINS:
                return criteriaBuilder.like(criteriaBuilder.toString(property),
    ...
    private static <T> Predicate handleTextFilter(Path propertyPath, String filterValue,
        MatchMode textFilterMatchMode, CriteriaBuilder criteriaBuilder) {
    ...
    private static <T> Predicate handleDateFilter(Path propertyPath, String filterValue,
        MatchMode textFilterMatchMode, CriteriaBuilder criteriaBuilder) {
    ...
    private static boolean isNumericPath(Class<?> propertyType) {
        List<Class<?>> numericTypes = Arrays.asList(int.class, Integer.class, long.class, Long.class,
